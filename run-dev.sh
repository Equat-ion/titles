#!/usr/bin/env bash
# Quick development run script for Ticket Booth
...
pip install --quiet PyGObject  # Needed for GTK bindings
...
cat > "$BUILD_DIR/ticketbooth.gresource.xml" << 'EOF'
...
cat > "$SCRIPT_DIR/src/shared.py" << 'EOF'
...
cat > "$BUILD_DIR/launcher.py" << 'LAUNCHER_EOF'
#!/usr/bin/env python3
import os
import sys
import signal
...
echo ""
export GSETTINGS_SCHEMA_DIR="$BUILD_DIR/schemas"
